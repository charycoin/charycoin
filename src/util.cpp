...
const char * const BITCOIN_CONF_FILENAME = "charycoin.conf";
const char * const BITCOIN_PID_FILENAME = "charycoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "charycoin" is a composite category enabling all CharyCoin-related debug output
            if(ptrCategory->count(string("charycoin"))) {
...
#else
    const char* pszModule = "charycoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CharyCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CharyCoinCore
    // Mac: ~/Library/Application Support/CharyCoinCore
    // Unix: ~/.charycoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CharyCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CharyCoinCore";
#else
    // Unix
    return pathRet / ".charycoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty charycoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override charycoin.conf
